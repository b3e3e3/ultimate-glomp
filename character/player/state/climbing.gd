...
@export var jump_off_force = Vector2(0, -400)
...
func on_physics_update(_delta: float) -> void:
	...
	if check_for_climbing() or character.velocity.y != 0:
		var ver := controller.get_vertical_input()
		character.vertical_move(ver, 200 if ver > 0 else 150)

	# climb side swapping
	elif character.direction.x == hor and hor != 0 \
	and controller.just_pressed_horizontal() \
	and climb_body and climb_body.is_in_group(&"ClimbSwappable"):
